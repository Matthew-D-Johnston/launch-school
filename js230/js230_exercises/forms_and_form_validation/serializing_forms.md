...
<html lang="en-US">
  <head>
    ...
    <meta charset="UTF-8">
    <link rel="stylesheet" href="serializing_forms.css">
    <script src="serializing_forms.js"></script>
  </head>
  <body>
    <div>
      <h1>Sign up Form</h1>
      <form novalidate>
        ...
        <fieldset>
          <dl>
            ...
            <dd>
              <input type="text" id="credit1" name="credit" minlength="4" maxlength="4" pattern="\d{4}"> -
              <input type="text" id="credit2" name="credit" minlength="4" maxlength="4" pattern="\d{4}"> -
              <input type="text" id="credit3" name="credit" minlength="4" maxlength="4" pattern="\d{4}"> -
              <input type="text" id="credit4" name="credit" minlength="4" maxlength="4" pattern="\d{4}">
              <p class="error-message"></p>
            </dd>
          </dl>
        </fieldset>
...
let App = {
  ...
  obtainFormData: function() {
    let form = document.querySelector('form');
    let formData = new FormData(form);
    return formData;
  },

  showSerializedData: function() {
    document.querySelector('form').addEventListener('submit', event => {
      event.preventDefault();

      if (this.isFirstNameValid() && this.isLastNameValid() && this.isEmailValid() && this.isPasswordValid() && this.isPhoneNumberValid() && this.isCreditCardValid()) {
        let formData = this.obtainFormData();
        let firstName = encodeURIComponent(formData.get('first-name'));
        let lastName = encodeURIComponent(formData.get('last-name'));
        let email = encodeURIComponent(formData.get('email'));
        let password = encodeURIComponent(formData.get('password'));
        let phoneNumber = encodeURIComponent(formData.get('phone'));
        let creditCard = encodeURIComponent(formData.getAll('credit').join(''));

        let serializedData = `first_name=${firstName}&last_name=${lastName}&email=${email}&password=${password}&phone=${phoneNumber}&credit_card=${creditCard}`;
        let serializedP = document.createElement('p');
        serializedP.innerText = serializedData;

        document.querySelector('div.serialize').appendChild(serializedP);
      }
    });
  },
