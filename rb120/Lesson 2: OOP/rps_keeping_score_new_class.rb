...
class RPSGame
  ...
  def display_score
    puts "#{human.name} has #{human_score.value} points."
    puts "#{computer.name} has #{computer_score.value} points."
  end

  def declare_winner
    if human_score.value == 10
      puts "#{human.name} is the first one to 10 points and wins the game!"
    elsif computer_score.value == 10
  ...
  def game_over?
    human_score.value == 10 || computer_score.value == 10
  end
